...
private:
    struct Node {
        ...
        std::weak_ptr<Node> prev; // <-- The crucial fix to break the cycle
...
public:
    ...
    void push_back(int value) {
        ...
        if (head_ == nullptr) {
            ...
            // Forward link (shared ownership)
            tail_->next = new_node;
            // Backward link (weak observation)
            new_node->prev = tail_;
            tail_ = new_node;
        }
    }
    ...
    void print_backward(std::ostream& os) const {
        ...
        while (current != nullptr) {
            ...
            // Convert weak_ptr to shared_ptr temporarily using .lock()
